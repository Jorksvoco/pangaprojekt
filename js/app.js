...
const STORAGE_KEY = "pangasusteem-state";
...
function loadState() {
  ...
  try {
    ...
    state.accounts = state.accounts.filter((a) => a.type !== "LAEN");
...
function txnTypeLabel(t) {
  const m = {
    KREDIT: "Sissemaks",
    DEEBET: "Väljavõtt",
    ÜLEKAN_SISSE: "Ülekan sisse",
    ÜLEKAN_VÄLJA: "Ülekan välja",
    LAEN_VÄLJA: "Laen välja",
    LAEN_TAGASI: "Laenu maks",
    INTRESS: "Intress",
  };
  return m[t] || t;
}
function txnBadgeClass(t) {
  if (["KREDIT", "ÜLEKAN_SISSE", "LAEN_VÄLJA"].includes(t))
    return "badge-green";
...
function showPage(id) {
  ...
  if (id === "transfer" || id === "deposit" || id === "withdraw")
  ...
  if (id === "new-loan") {
    populateClientSelects("nl-klient");
    updateNewLoanAccountOptions();
  }
  if (id === "loan-payment") {
    populateLoanSelects();
    populateAccountSelects("lp-konto", "ARVELDUS");
  }
...
function renderDashboard() {
  document.getElementById("stat-clients").textContent = state.clients.filter(
    (c) => c.active,
  ).length;
  document.getElementById("stat-accounts").textContent = state.accounts.filter(
    (a) => a.active,
  ).length;
  document.getElementById("stat-txns").textContent = state.transactions.length;
  document.getElementById("stat-loans").textContent = state.loans.filter(
    (l) => l.status === "AKTIIVNE",
  ).length;
  ...
  if (!recent.length) {
    tbody.innerHTML =
      '<tr><td colspan="4" class="empty">Tehinguid pole</td></tr>';
    return;
  }
  ...
  const accs = state.accounts
    .filter((a) => a.active && a.type !== "LAEN")
    .slice(0, 4);
  if (!accs.length) {
    mini.innerHTML =
      '<div class="empty"><div class="empty-icon">🏦</div>Kontosid pole</div>';
    return;
  }
  mini.innerHTML = accs
    .map((a) => {
...
function addClient() {
  ...
  const fn = e("ac-eesnimi"),
    ...
    addr = e("ac-aadress");
  if (!fn || !ln || !ic || !em) {
    toast("Täida kohustuslikud väljad", "error");
    return;
  }
  if (state.clients.find((c) => c.isikukood === ic)) {
    toast("Selle isikukoodiga klient on juba olemas", "error");
    return;
  }
...
function renderClients() {
  ...
  if (!state.clients.length) {
    tbody.innerHTML =
      '<tr><td colspan="6" class="empty">Kliente pole</td></tr>';
    return;
  }
...
["ac-eesnimi", "ac-perenimi", "ac-isikukood", "ac-email"].forEach((id) => {
  setTimeout(() => {
    ...
    if (el) el.addEventListener("input", updateAddClientSQL);
  }, 100);
});
...
function renderAccounts() {
  ...
  if (!state.accounts.filter((a) => a.active && a.type !== "LAEN").length) {
    grid.innerHTML =
      '<div class="empty" style="grid-column:1/-1"><div class="empty-icon">🏦</div>Kontosid pole</div>';
    return;
  }
...
function populateClientSelects(id = "nl-klient") {
  ...
  sel.innerHTML = state.clients
    ...
    .join("");
  if (!sel.innerHTML)
    sel.innerHTML = '<option value="">— Pole kliente —</option>';
}

function updateNewLoanAccountOptions() {
  const clientID = parseInt(document.getElementById("nl-klient")?.value);
  const sel = document.getElementById("nl-konto");
  if (!sel) return;
  const accounts = state.accounts.filter(
    (a) => a.active && a.clientID === clientID && a.type === "ARVELDUS",
  );
  sel.innerHTML = accounts
    .map(
      (a) =>
        `<option value="${a.id}">${a.nr.substring(0, 10)}... (${fmt(a.balance)} EUR)</option>`,
    )
    .join("");
  if (!sel.innerHTML)
    sel.innerHTML =
      '<option value="">No settlement account for this client</option>';
}
...
function populateAccountSelects(id = null, filterType = null) {
  ...
  ids.forEach((selId) => {
    ...
    const accs = state.accounts.filter(
      (a) => a.active && (!filterType || a.type === filterType),
    );
    sel.innerHTML = accs
      .map((a) => {
        const cl = state.clients.find((c) => c.id === a.clientID);
      ...
      .join("");
    if (!sel.innerHTML)
      sel.innerHTML = '<option value="">— Pole kontosid —</option>';
  });
}
...
function doDeposit() {
  ...
  if (!accID || !amount || amount <= 0) {
    toast("Vali konto ja sisesta summa", "error");
    return;
  }
...
function doWithdraw() {
  ...
  if (acc.balance < amount) {
    toast("Ebapiisav saldo! (" + fmt(acc.balance) + " €)", "error");
    return;
  }
  acc.balance = Math.round((acc.balance - amount) * 100) / 100;
...
function doTransfer() {
  ...
  if (!fromID || !toID || !amount || amount <= 0) {
    toast("Täida kõik väljad", "error");
    return;
  }
  if (fromID === toID) {
    toast("Saatja ja saaja ei saa olla sama konto", "error");
    return;
  }
  const from = state.accounts.find((a) => a.id === fromID);
  const to = state.accounts.find((a) => a.id === toID);
  if (from.balance < amount) {
    toast("Ebapiisav saldo! (" + fmt(from.balance) + " €)", "error");
    return;
  }
...
function updateDepositSQL() {
  ...
  document.getElementById("deposit-sql-code").innerHTML = hlSQL(
    `EXEC dbo.sp_Sissemaks\n  @KontoID   = ${k || "?"},\n  @Summa     = ${s},\n  @Kirjeldus = '${d}';`,
  );
}
function updateWithdrawSQL() {
  ...
  document.getElementById("withdraw-sql-code").innerHTML = hlSQL(
    `EXEC dbo.sp_Väljavõtt\n  @KontoID   = ${k || "?"},\n  @Summa     = ${s},\n  @Kirjeldus = '${d}';`,
  );
}
...
function populateHistoryFilters() {
  ...
  const cur = sel.value;
  sel.innerHTML =
    '<option value="">Kõik kontod</option>' +
    state.accounts
      .map((a) => {
        const cl = state.clients.find((c) => c.id === a.clientID);
...
function renderHistory() {
  ...
  if (!txns.length) {
    tbody.innerHTML =
      '<tr><td colspan="7" class="empty">Tehinguid pole</td></tr>';
    return;
  }
...
function calcLoan() {
  ...
  if (!P || isNaN(rate) || !n) {
    ...
    document.getElementById("loan-schedule-preview").innerHTML =
      '<tr><td colspan="5" style="color:var(--muted);padding:12px">Täida vorm eelvaate jaoks</td></tr>';
    document.getElementById("new-loan-sql").innerHTML = hlSQL("-- Täida vorm");
    return;
  }
  ...
  for (let i = 1; i <= n; i++) {
    ...
    if (i <= 5 || i === n) {
      if (i === n && n > 6)
        rows += `<tr><td colspan="5" style="color:var(--muted);text-align:center;padding:6px">… ${n - 6} rida …</td></tr>`;
...
function renderLoans() {
  ...
  if (!state.loans.length) {
    container.innerHTML =
      '<div class="empty"><div class="empty-icon">💰</div>Laene pole</div>';
    return;
  }
...
function populateLoanSelects() {
  ...
  sel.innerHTML = activeLoans
    ...
    .join("");
  if (!sel.innerHTML)
    sel.innerHTML = '<option value="">— Pole aktiivseid laene —</option>';
  updateLoanPaySQL();
}
...
function updateLoanPaySQL() {
  ...
  const next = loan
    ? (state.loanSchedules[loan.id] || []).find((s) => !s.paid)
    : null;
  if (next) {
    document.getElementById("lp-info").innerHTML =
      `Järgmine maks nr ${next.nr}: põhiosa <b>${fmt(next.principal)} €</b> + intress <b>${fmt(next.interest)} €</b> = <b style="color:var(--accent)">${fmt(next.total)} €</b>`;
  } else {
    document.getElementById("lp-info").textContent = "—";
  }
...
function doLoanPayment() {
  ...
  if (!loanID || !accID) {
    toast("Vali laen ja konto", "error");
    return;
  }
  const loan = state.loans.find((l) => l.id === loanID);
  if (!loan || loan.status !== "AKTIIVNE") {
    toast("Valitud laen ei ole aktiivne", "error");
    return;
  }
  const schedule = state.loanSchedules[loanID] || [];
  const next = schedule.find((s) => !s.paid);
  if (!next) {
    toast("Kõik maksed on tasutud!");
    return;
  }
  const acc = state.accounts.find((a) => a.id === accID);
  if (!acc || !acc.active) {
    toast("Valitud konto ei ole saadaval", "error");
    return;
  }
  if (acc.type !== "ARVELDUS") {
    toast("Makseks peab kasutama arvelduskontot", "error");
    return;
  }
  if (acc.balance < next.total) {
    toast("Ebapiisav saldo! (" + fmt(acc.balance) + " €)", "error");
    return;
  }
  const loanAcc = state.accounts.find((a) => a.id === loan.accountID);
  // Teosta maks
  acc.balance = Math.round((acc.balance - next.total) * 100) / 100;
  loan.remaining = Math.max(
    0,
    Math.round((loan.remaining - next.principal) * 100) / 100,
  );
  if (loanAcc) loanAcc.balance = loan.remaining;
  next.paid = true;
  addTxn(
    accID,
    "LAEN_TAGASI",
    next.total,
    "Laen #" + loanID + " maks #" + next.nr,
  );
  if (!schedule.find((s) => !s.paid)) {
    loan.status = "SULETUD";
    loan.remaining = 0;
    if (loanAcc) loanAcc.balance = 0;
  }
...
function renderSQLLog() {
  ...
  document.getElementById("sql-log-count").textContent =
    state.sqlLog.length + " lauset";
  if (!state.sqlLog.length) {
    list.innerHTML = '<div class="empty">SQL logi on tühi</div>';
    return;
  }
  list.innerHTML = state.sqlLog
    .map(
      (entry, i) => `
...
function seedDemo() {
  ...
  state.clients.push({
    ...
    firstName: "Aleks",
    lastName: "Rohtla",
    isikukood: "49001011234",
    email: "aleks@email.ee",
    phone: "+372 5000 0001",
    address: "Tallinn",
    created: now(),
    active: true,
  ...
  state.clients.push({
    ...
    firstName: "Jorgen",
    lastName: "Siimsoo",
    isikukood: "38505050505",
    email: "jorgen@email.ee",
    phone: "+372 5000 0002",
    address: "Tartu",
    created: now(),
    active: true,
...
function calcLoan() {
  ...
  const rate = parseFloat(document.getElementById("nl-intress").value);
  const n = parseInt(document.getElementById("nl-tahtaeg").value);
  const payoutAccID = document.getElementById("nl-konto")?.value;
  if (!P || isNaN(rate) || !n) {
    document.getElementById("loan-preview").style.display = "none";
    document.getElementById("loan-monthly").textContent = "—";
    document.getElementById("loan-schedule-preview").innerHTML =
      '<tr><td colspan="5" style="color:var(--muted);padding:12px">Fill out the form to preview the schedule</td></tr>';
    document.getElementById("new-loan-sql").innerHTML = hlSQL(
      "-- Fill out the form",
    );
    return;
  }
  const M = calcAnnuity(P, rate, n);
  document.getElementById("loan-monthly").textContent = fmt(M) + " EUR";
  document.getElementById("loan-preview").style.display = "";
  const tbody = document.getElementById("loan-schedule-preview");
  const r = rate / 100 / 12;
  let bal = P;
  ...
  for (let i = 1; i <= n; i++) {
    ...
    if (i <= 5 || i === n) {
      if (i === n && n > 6) {
        rows += `<tr><td colspan="5" style="color:var(--muted);text-align:center;padding:6px">... ${n - 6} rows ...</td></tr>`;
      }
      rows += `<tr><td>${i}</td><td>${fmt(principal)}</td><td>${fmt(interest)}</td><td>${fmt(M)}</td><td>${fmt(Math.max(0, bal - principal))}</td></tr>`;
    }
    bal = Math.max(0, bal - principal);
  ...
  tbody.innerHTML = rows;
  const sql = `DECLARE @LaenID INT;\nEXEC dbo.sp_ValjastLaen\n  @KlientID         = ${document.getElementById("nl-klient").value || "?"},\n  @ArveldusKontoID  = ${payoutAccID || "?"},\n  @Summa            = ${P.toFixed(2)},\n  @IntressiMaarA    = ${(rate / 100).toFixed(4)},\n  @TahtaegKuudes    = ${n},\n  @LaenID           = @LaenID OUTPUT;\n-- Kuumaks: ${fmt(M)} EUR`;
  document.getElementById("new-loan-sql").innerHTML = hlSQL(sql);
}

function doNewLoan() {
  const clientID = parseInt(document.getElementById("nl-klient").value);
  const payoutAccID = parseInt(document.getElementById("nl-konto").value);
  const P = parseFloat(document.getElementById("nl-summa").value);
  const rate = parseFloat(document.getElementById("nl-intress").value);
  const n = parseInt(document.getElementById("nl-tahtaeg").value);
  if (!clientID || !payoutAccID || !P || isNaN(rate) || !n) {
    toast("Fill out all loan fields", "error");
    return;
  }
  const cl = state.clients.find((c) => c.id === clientID);
  const payoutAcc = state.accounts.find((a) => a.id === payoutAccID);
  if (
    !payoutAcc ||
    !payoutAcc.active ||
    payoutAcc.clientID !== clientID ||
    payoutAcc.type !== "ARVELDUS"
  ) {
    toast("Choose an existing settlement account for this client", "error");
    return;
  }
  payoutAcc.balance = Math.round((payoutAcc.balance + P) * 100) / 100;
  const M = Math.round(calcAnnuity(P, rate, n) * 100) / 100;
  const r = rate / 100 / 12;
  let bal = P;
  const schedule = [];
  ...
  state.loanSchedules[loanID] = schedule;
  addTxn(payoutAccID, "LAEN_VÄLJA", P, "Laen #" + loanID);
  const sql = `DECLARE @LaenID INT;\nEXEC dbo.sp_ValjastLaen\n  @KlientID         = ${clientID},\n  @ArveldusKontoID  = ${payoutAccID},\n  @Summa            = ${P.toFixed(2)},\n  @IntressiMaarA    = ${(rate / 100).toFixed(4)},\n  @TahtaegKuudes    = ${n},\n  @LaenID           = @LaenID OUTPUT;\n-- Loan #${loanID} issued to ${cl ? cl.name : "client"} | Monthly payment: ${fmt(M)} EUR`;
  document.getElementById("new-loan-sql").innerHTML = hlSQL(sql);
  logSQL("Uus laen #" + loanID + " - " + (cl ? cl.name : "client"), sql);
  saveState();
  toast("Loan issued to existing account");
  renderDashboard();
  renderLoans();
  populateLoanSelects();
  updateNewLoanAccountOptions();
...
function doLoanPayment() {
  ...
  if (acc.balance < next.total) {
    toast("Ebapiisav saldo! (" + fmt(acc.balance) + " EUR)", "error");
    return;
  }
  acc.balance = Math.round((acc.balance - next.total) * 100) / 100;
  loan.remaining = Math.max(
  ...
  const sql = `EXEC dbo.sp_MaksaLaenMaks\n  @LaenID         = ${loanID},\n  @ArvelduKontoID = ${accID};\n-- Maks #${next.nr}: ${fmt(next.total)} EUR`;
  document.getElementById("loan-pay-sql").innerHTML = hlSQL(sql);
  logSQL("Laenu maks #" + next.nr + " - " + fmt(next.total) + " EUR", sql);
  saveState();
  toast("Laenu maks teostatud!");
  populateLoanSelects();
  populateAccountSelects("lp-konto", "ARVELDUS");
  updateNewLoanAccountOptions();
  renderDashboard();
